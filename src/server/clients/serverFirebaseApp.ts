...
export const serverFirebaseApp = () => {
  ...
  console.log("Unescaped credential: ", unescape(env.SERVICE_ACCOUNT_JSON));
  console.log("JSON Parsed credential: ", JSON.parse(unescape(env.SERVICE_ACCOUNT_JSON)));
  
  let credential: admin.credential.Credential;
  try {
    credential = admin.credential.cert(JSON.parse(unescape(env.SERVICE_ACCOUNT_JSON))); 
  } catch (e) {
